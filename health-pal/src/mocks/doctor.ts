import { TDoctorCard } from '@app/models/doctor'
import { TDoctorBanner } from '@app/types/doctor'

export const MOCK_DOCTORS: TDoctorCard[] = [
  {
    documentId: 'doc_001',
    id: 1,
    name: 'Dr. John Smith',
    email: 'john.smith@clinic.com',
    avatar: 'https://picsum.photos/id/91/3504/2336',
    specialty: 'Dentistry',
    address: '123 Smile St, Dental City, DC 12345',
    reviewCounter: 5,
    rating: 4.8,
    dayOfBirth: new Date('2001-02-02'),
    gender: true,
    nickname: 'doctor_001',
  },
  {
    documentId: 'doc_002',
    id: 2,
    name: 'Dr. Emily Carter',
    email: 'emily.carter@clinic.com',
    avatar: 'https://picsum.photos/id/91/3504/2336',
    specialty: 'Cardiology',
    address: '456 Heart Ave, Cardio Town, CT 67890',
    reviewCounter: 3,
    rating: 4.3,
    dayOfBirth: new Date('2001-02-02'),
    gender: true,
    nickname: 'doctor_002',
  },
  {
    documentId: 'doc_003',
    id: 3,
    name: 'Dr. Michael Brown',
    email: 'michael.brown@clinic.com',
    avatar: 'https://picsum.photos/id/91/3504/2336',
    specialty: 'Pulmonology',
    address: '789 Lung Rd, Breath City, BC 23456',
    reviewCounter: 2,
    rating: 4.0,
    dayOfBirth: new Date('2001-02-02'),
    gender: true,
    nickname: 'doctor_003',
  },
  {
    documentId: 'doc_004',
    id: 4,
    name: 'Dr. Sarah Davis',
    email: 'sarah.davis@clinic.com',
    avatar: 'https://picsum.photos/id/91/3504/2336',
    specialty: 'General',
    address: '101 Health St, Wellness City, WC 34567',
    reviewCounter: 4,
    rating: 4.5,
    dayOfBirth: new Date('2001-02-02'),
    gender: true,
    nickname: 'doctor_004',
  },
  {
    documentId: 'doc_005',
    id: 5,
    name: 'Dr. Robert Wilson',
    email: 'robert.wilson@clinic.com',
    avatar: 'https://picsum.photos/id/91/3504/2336',
    specialty: 'Neurology',
    address: '202 Brain Ave, Neuro Town, NT 45678',
    reviewCounter: 1,
    rating: 3.8,
    dayOfBirth: new Date('2001-02-02'),
    gender: true,
    nickname: 'doctor_005',
  },
  {
    documentId: 'doc_006',
    id: 6,
    name: 'Dr. Linda Moore',
    email: 'linda.moore@clinic.com',
    avatar: 'https://picsum.photos/id/91/3504/2336',
    specialty: 'Gastroenterology',
    address: '303 Gut Rd, Digestive City, DC 56789',
    reviewCounter: 0,
    rating: 0,
    reviews: [],
    dayOfBirth: new Date('2001-02-02'),
    gender: true,
    nickname: 'doctor_006',
  },
  {
    documentId: 'doc_007',
    id: 7,
    name: 'Dr. James Taylor',
    email: 'james.taylor@clinic.com',
    avatar: 'https://picsum.photos/id/91/3504/2336',
    specialty: 'Laboratory',
    address: '404 Lab St, Test City, TC 67890',
    reviewCounter: 2,
    rating: 4.2,
    dayOfBirth: new Date('2001-02-02'),
    gender: true,
    nickname: 'doctor_007',
  },
  {
    documentId: 'doc_008',
    id: 8,
    name: 'Dr. Anna Lee',
    email: 'anna.lee@clinic.com',
    avatar: 'https://picsum.photos/id/91/3504/2336',
    specialty: 'Vaccination',
    address: '505 Vaccine Ave, Immune City, IC 78901',
    reviewCounter: 3,
    rating: 4.7,
    dayOfBirth: new Date('2001-02-02'),
    gender: true,
    nickname: 'doctor_008',
  },
  {
    documentId: 'doc_009',
    id: 9,
    name: 'Dr. David Kim',
    email: 'david.kim@clinic.com',
    avatar: 'https://picsum.photos/id/91/3504/2336',
    specialty: 'Dentistry',
    address: '606 Tooth Rd, Smile Town, ST 89012',
    reviewCounter: 1,
    rating: 4.0,
    dayOfBirth: new Date('2001-02-02'),
    gender: true,
    nickname: 'doctor_009',
  },
  {
    documentId: 'doc_010',
    id: 10,
    name: 'Dr. Maria Garcia',
    email: 'maria.garcia@clinic.com',
    avatar: 'https://picsum.photos/id/91/3504/2336',
    specialty: 'Cardiology',
    address: '707 Pulse St, Heart City, HC 90123',
    reviewCounter: 0,
    rating: 0,
    reviews: [],
    dayOfBirth: new Date('2001-02-02'),
    gender: true,
    nickname: 'doctor_010',
  },
]

export const DOCTOR_BANNER: TDoctorBanner[] = [...new Array(6).keys()].map(() => ({
  title: `Looking for\nSpecialist Doctors?`,
  description: `Schedule an appointment with\nour top doctors.`,
}))
